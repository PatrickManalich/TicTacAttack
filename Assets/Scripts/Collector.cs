...
public class Collector : MonoBehaviour {
    ...
    private Transform parentTransform;  // The transform of the collector's parent, or the OVRCameraRig
    ...
    public void Damage(int damage) {
        health -= damage;
        if(health <= 0) {
            Debug.Log("The Collector has been destroyed.");
        }
    }
    ...
    private void Update() {
        if (OVRInput.Get(OVRInput.Button.SecondaryThumbstickLeft) || OVRInput.Get(OVRInput.Button.PrimaryThumbstickLeft)) {
            if (readyToSnapTurn) {  // If ready to snap turn left
                ...
                parentTransform.rotation = Quaternion.Euler(euler);
